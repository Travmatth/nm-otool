...
/*   Updated: 2020/02/06 22:08:41 by tmatthew         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		validate_i386_sections(char *file, struct segment_command_64 *segment, ptrdiff_t base, int flags)
{
	int					status;
	uint32_t			i;
	uint32_t			offset;
...
static int		validate_segment(char *file, t_ctx *ctx, struct segment_command_64 *seg, int flags)
{
	...
	if ((segment.nsects * sizeof(struct section) >
		...
		return (EXIT_FAILURE);
	return (validate_i386_sections(file, &segment, (char*)seg - file, flags));
}
...
int		validate_mach_x86_64(char *file, t_ctx *ctx, int flags)
{
	...
	while (i++ < header.ncmds)
	{
		...
		if ((lc.cmdsize % 4)
			...
			, (struct segment_command_64*)(file + offset), flags) == EXIT_FAILURE)
